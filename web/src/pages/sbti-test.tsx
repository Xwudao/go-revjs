...
// ── Images ──────────────────────────────────────────────────────────────────
import imgATMer from '@/assets/sbti/ATM-er.png';
import imgBOSS from '@/assets/sbti/BOSS.png';
import imgCTRL from '@/assets/sbti/CTRL.png';
import imgDEAD from '@/assets/sbti/DEAD.png';
import imgDiors from '@/assets/sbti/Dior-s.jpg';
import imgDRUNK from '@/assets/sbti/DRUNK.png';
import imgFAKE from '@/assets/sbti/FAKE.png';
import imgFUCK from '@/assets/sbti/FUCK.png';
import imgGOGO from '@/assets/sbti/GOGO.png';
import imgHHHH from '@/assets/sbti/HHHH.png';
import imgIMFW from '@/assets/sbti/IMFW.png';
import imgIMSB from '@/assets/sbti/IMSB.png';
import imgJOKER from '@/assets/sbti/JOKE-R.jpg';
import imgLOVER from '@/assets/sbti/LOVE-R.png';
import imgMALO from '@/assets/sbti/MALO.png';
import imgMONK from '@/assets/sbti/MONK.png';
import imgMUM from '@/assets/sbti/MUM.png';
import imgOHNO from '@/assets/sbti/OH-NO.png';
import imgOJBK from '@/assets/sbti/OJBK.png';
import imgPOOR from '@/assets/sbti/POOR.png';
import imgSEXY from '@/assets/sbti/SEXY.png';
import imgSHIT from '@/assets/sbti/SHIT.png';
import imgSOLO from '@/assets/sbti/SOLO.png';
import imgTHANK from '@/assets/sbti/THAN-K.png';
import imgTHINK from '@/assets/sbti/THIN-K.png';
import imgWOC from '@/assets/sbti/WOC.png';
import imgZZZZ from '@/assets/sbti/ZZZZ.png';
...
const TYPE_LIBRARY: Record<string, TypeDef> = {
  CTRL: {
    code: 'CTRL',
    cn: '拿捏者',
    intro: '怎么样，被我拿捏了吧？',
...
const DIMENSION_META: Record<string, { name: string; model: string }> = {
...
const SPECIAL_DRINK_GATE: Question = {
  id: 'drink_gate_q1',
  special: true,
  kind: 'drink_gate',
  dim: '',
...
const SPECIAL_DRINK_TRIGGER: Question = {
  id: 'drink_gate_q2',
  special: true,
  kind: 'drink_trigger',
  dim: '',
...
export default function SbtiTestPage() {
  ...
  const getVisible = useCallback(
    (qs: Question[], ans: Record<string, number>): Question[] => {
      const visible = [...qs];
      const gateIdx = visible.findIndex((q) => q.id === 'drink_gate_q1');
      if (gateIdx !== -1 && ans['drink_gate_q1'] === 3) {
        visible.splice(gateIdx + 1, 0, SPECIAL_DRINK_TRIGGER);
      }
      return visible;
    },
    [],
  ...
  const submitTest = useCallback(() => {
    ...
    QUESTIONS.forEach((q) => {
      rawScores[q.dim] = (rawScores[q.dim] ?? 0) + (answers[q.id] ?? 0);
    });

    DIMENSION_ORDER.forEach((d) => {
      levels[d] = sumToLevel(rawScores[d]);
    });

    const userVector = DIMENSION_ORDER.map((d) => levelNum(levels[d]));

    const ranked = NORMAL_TYPES.map((t) => {
      const vec = parsePattern(t.pattern).map(levelNum);
      let distance = 0;
      let exact = 0;
    ...
    if (drunkTriggered) {
      ...
      finalType = TYPE_LIBRARY[best.code];
    }

    setResult({
      rawScores,
      levels: levels as Record<string, 'L' | 'M' | 'H'>,
      finalType,
      modeKicker,
      badge,
      sub,
  ...
  const visibleQuestions = getVisible(shuffledQuestions, answers);
      ...
      <header className={classes.header}>
        <div className={classes.headerMeta}>
          <span className={classes.kicker}>
            <span className="i-mdi-brain" aria-hidden="true" />
            人格测试
          </span>
        </div>
        <h1 className={classes.title}>SBTI 人格测试</h1>
        <p className={classes.subtitle}>
          MBTI 已经过时，SBTI 来了。回答 30 道题，揭晓你的人格类型。
        </p>
        ...
        <div className={classes.testCard}>
              ...
              >
                ...
                <div className={classes.options}>
                  {q.options.map((opt, i) => {
                    ...
                    return (
                      <label key={opt.value} className={classes.optionLabel}>
                        <input
                          type="radio"
                          name={q.id}
                          value={opt.value}
          ...
          <div className={classes.actionsBottom}>
            ...
            <div className={classes.actionsRight}>
              <button className={classes.btnSecondary} onClick={() => setScreen('intro')}>
                返回首页
              </button>
              <button
        ...
        <div className={classes.resultCard}>
          <div className={classes.resultTop}>
            <div
              className={clsx(
                classes.posterBox,
                !TYPE_IMAGES[result.finalType.code] && classes.noPoster,
              )}
            >
              ...
              <p className={classes.posterCaption}>{result.finalType.intro}</p>
            </div>

            <div className={classes.typeBox}>
              <div className={classes.typeKicker}>{result.modeKicker}</div>
              <h2 className={classes.typeName}>
                {result.finalType.code}（{result.finalType.cn}）
              </h2>
              <span className={classes.matchBadge}>{result.badge}</span>
              <p className={classes.typeSub}>{result.sub}</p>
            </div>
          </div>
          ...
          <details className={classes.authorBox}>
            <summary className={classes.authorSummary}>
              <span>
                <span className="i-mdi-account-circle-outline" aria-hidden="true" />
                作者的话
              </span>
              <span className={classes.authorToggle}>展开 / 收起</span>
            </summary>
          ...
          <div className={classes.resultActions}>
            <div className={classes.actionsRight}>
              <button className={classes.btnSecondary} onClick={startTest}>
                重新测试
              </button>
              <button className={classes.btnPrimary} onClick={() => setScreen('intro')}>
                回到首页
              </button>
            </div>
          </div>

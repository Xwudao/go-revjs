...
test('loose equality', () => expectJS('null == x').toMatchInlineSnapshot('x == null;'));

test('strict inequality', () =>
  expectJS('"red" !== color').toMatchInlineSnapshot('color !== "red";'));

test('loose inequality', () => expectJS('null != x').toMatchInlineSnapshot('x != null;'));
...
test('greater or equal', () => expectJS('0 >= x').toMatchInlineSnapshot('x <= 0;'));
...
test('string', () => expectJS('"str" == x').toMatchInlineSnapshot('x == "str";'));

test('number', () => expectJS('1 == x').toMatchInlineSnapshot('x == 1;'));

test('negative number', () => expectJS('-1 == x').toMatchInlineSnapshot('x == -1;'));
...
test('ignore other operators', () => expectJS('2 + x').toMatchInlineSnapshot('2 + x;'));

...
import traverse, { visitors } from '../interop/babel-traverse';
...
export async function applyTransformAsync<TOptions>(
  ...
  if (transform.visitor) traverse(ast, transform.visitor(options), undefined, state);
...
export function applyTransforms(
  ...
  if (traverseOptions.length > 0) {
    const visitor: TraverseOptions<TransformState> = visitors.merge(traverseOptions);
